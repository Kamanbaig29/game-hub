 ...
 * 
 * This test is configured for high-load stress testing:
 * - Up to 2000 concurrent virtual users
 * - Total test duration: ~23 minutes
 * - Reduced sleep times (0.2s) for maximum load generation
 * - Relaxed thresholds for stress testing scenarios
 * 
 * WARNING: This will generate significant load on your server.
 * Make sure your server infrastructure can handle this load.
 */
...
// Test configuration - Aggressive stress test with 2000 users
export const options = {
  stages: [
    ...
    { duration: '5m', target: 2000 },  // Stay at 2000 users for 5 minutes (stress test)
    { duration: '2m', target: 1000 },  // Ramp down to 1000 users
    { duration: '2m', target: 500 },   // Ramp down to 500 users
    { duration: '2m', target: 0 },     // Ramp down to 0 users
  ],
  thresholds: {
    http_req_duration: ['p(95)<5000'], // 95% of requests should be below 5s (relaxed for high load)
    http_req_failed: ['rate<0.15'],    // Error rate should be less than 15% (relaxed for stress test)
    errors: ['rate<0.15'],
  },
};
...
export default function (data) {
  ...
  if (!authToken) {
    ...
    sleep(0.2); // Reduced sleep for aggressive stress test
  }
